...
if 'debian' in name:
  program, command = ['/bin/sh', ['-c', 'cd /tmp && wget -O extend.sh http://file.autovm.net/machine/debian/extend.sh && sh extend.sh && rm extend.sh']]

elif 'ubuntu' in name:
  program, command = ['/bin/sh', ['-c', 'curl http://file.autovm.net/machine/ubuntu/extend.sh | sh']]
  
elif 'centos 6.8' in name:
  program, command = ['/bin/sh', ['-c', 'curl http://file.autovm.net/machine/centos/extend.sh | sh']]
