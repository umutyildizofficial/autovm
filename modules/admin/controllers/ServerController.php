...
class ServerController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            if ($model->save(false)) {
                Yii::$app->session->addFlash('success', Yii::t('app', 'Yeni fiziksel sunucu kaydı başarıyla oluşturuldu'));
    ...
    public function actionEdit($id)
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            if ($model->save(false)) {
                Yii::$app->session->addFlash('success', Yii::t('app', 'Fiziksel sunucu düzenlendi'));
    ...
    public function actionDelete()
    {
        ...
        foreach ($data as $id) {
            ...
            if ($server) {
                ...
                if ($deleted) {
                    Log::log(sprintf('%s fiziksel sunucusu %s tarafından silindi', $server->ip, Yii::$app->user->identity->fullName));   
                }
            }
        }
